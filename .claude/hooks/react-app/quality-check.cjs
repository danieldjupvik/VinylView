...
class TypeScriptConfigCache {
  ...
  rebuild() {
    ...
    const configPriority = [
      ...
      'tsconfig.json', // Base config (fallback)
    ];
  ...
  getTsConfigForFile(filePath) {
    ...
    if (relativePath.startsWith('api/') || relativePath.startsWith('api\\')) {
      const serverConfig = path.join(projectRoot, 'tsconfig.server.json');
      if (require('fs').existsSync(serverConfig)) {
        return serverConfig;
      }
    }
    ...
    try {
      ...
      if (debuggerRule.enabled !== false) {
        lines.forEach((line, index) => {
          ...
          // Match 'debugger' as a statement
          if (/\bdebugger\b/.test(line)) {
            const severity = debuggerRule.severity || 'error';
            const message = debuggerRule.message || 'Remove debugger statements before committing';
...
async function main() {
  ...
  if (editedFileErrors.length > 0) {
    ...
    console.error(`${colors.yellow}  1. Fix ALL issues listed above${colors.reset}`);
    console.error(`${colors.yellow}  2. This hook will run again automatically after your fix${colors.reset}`);
    console.error(`${colors.yellow}  3. Continue with your original task once all checks pass${colors.reset}`);
    console.error(`${colors.cyan}💡 TIPS:${colors.reset}`);
    console.error(`${colors.cyan}  → Check project docs (CLAUDE.md, README) for coding conventions${colors.reset}`);
    console.error(`${colors.cyan}  → Ensure proper TypeScript types (avoid 'as any')${colors.reset}`);
    console.error(`${colors.cyan}  → Remove unused variables and imports${colors.reset}`);
    process.exit(2);
